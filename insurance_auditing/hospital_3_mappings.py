...
DEFAULT_MAPPING_PATH = Path("mappings/hospital_3_service_mappings.json")


def load_hospital_3_mapping_artifact(
    path: Path,
    ...
    try:
        ...
        raise ValueError(f"Hospital 3 mapping artifact does not exist: {path}") from exc
    except json.JSONDecodeError as exc:
        raise ValueError(f"Hospital 3 mapping artifact is invalid JSON: {path}") from exc
    if not isinstance(payload, dict):
        raise ValueError("Hospital 3 mapping artifact must be an object")
    if payload.get("schema_version") != MAPPING_SCHEMA_VERSION:
        raise ValueError("unsupported Hospital 3 mapping schema version")
    if payload.get("hospital") != 3:
        raise ValueError("mapping artifact is not for Hospital 3")
    if not contract.source_sha256 or payload.get("contract_sha256") != contract.source_sha256:
        raise ValueError("Hospital 3 mapping artifact has a stale contract fingerprint")
    if payload.get("review_method") != "aggregate_contract_rate_tiebreaker":
        raise ValueError("Hospital 3 mapping artifact has an unsupported review method")
    prompt_path = payload.get("prompt_path")
    prompt_sha256 = payload.get("prompt_sha256")
    if not isinstance(prompt_path, str) or not isinstance(prompt_sha256, str):
        raise ValueError("Hospital 3 mapping artifact is missing prompt provenance")
    resolved_prompt = path.parent.parent / prompt_path
    try:
        actual_prompt_sha256 = hashlib.sha256(resolved_prompt.read_bytes()).hexdigest()
    except FileNotFoundError as exc:
        raise ValueError(
            f"Hospital 3 mapping prompt does not exist: {resolved_prompt}"
        ) from exc
    if actual_prompt_sha256 != prompt_sha256:
        raise ValueError("Hospital 3 mapping prompt fingerprint is stale")
    entries = payload.get("mappings")
    if not isinstance(entries, list):
        raise ValueError("Hospital 3 mapping artifact must contain a mappings array")

    matcher = ServiceMatcher(contract)
    overrides: dict[str, ServiceMappingOverride] = {}
    for entry in entries:
        if not isinstance(entry, dict):
            raise ValueError("Hospital 3 mapping entries must be objects")
        ...
        if (
            ...
            raise ValueError("Hospital 3 mapping descriptions must be unique and normalized")
        if service_name not in contract.services:
            raise ValueError(f"Hospital 3 mapping references unknown service: {service_name!r}")
        if (
            isinstance(confidence, bool)
            or not isinstance(confidence, (int, float))
            or not 0.80 <= confidence <= 0.95
        ):
            raise ValueError(f"Hospital 3 mapping confidence is invalid for {key!r}")
        if service_name not in {
            candidate for _, candidate in matcher.rank_candidates(key, limit=5)
        }:
            raise ValueError(f"Hospital 3 mapping selection is outside candidates for {key!r}")
        if (
            ...
            raise ValueError(f"Hospital 3 mapping evidence is invalid for {key!r}")
